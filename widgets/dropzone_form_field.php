...
EOT;
    ...
    public function sanitize_field($value, $field)
    {
        $ret_arr = [];
        $user = wp_get_current_user();
        $user_id = (!isset($user) || !is_object($user) || !is_a($user, 'WP_User') || !$user->ID) ? 0 : $user->ID;
        ...
        // --- SUDOWP PATCH START (CVE-2024-5153) ---
        // Strictly sanitize the hash to allow only alphanumeric characters, dashes, and underscores.
        // This effectively neutralizes any Directory Traversal attempts (e.g., "../")
        $dropzone_hash = sanitize_key($dropzone_hash);

        if (empty($dropzone_hash)) {
            return ''; // Abort if hash is invalid or empty after sanitization
        }
        // --- SUDOWP PATCH END ---

        $uploads_dir_info = wp_upload_dir();
        // Safe to use glob now as $dropzone_hash is sanitized
        $target_folder = $uploads_dir_info['basedir'] . "/elementor/forms/" . $user_id . "/temp/" . $dropzone_hash . "/*";
        
        // Ensure we are only looking inside the expected directory structure
        $real_base = $uploads_dir_info['basedir'] . "/elementor/forms/" . $user_id . "/temp/";
        if (strpos($target_folder, $real_base) !== 0) {
        ...
        if ($files && count($files)) {
            ...
            $temp_folder = $uploads_dir_info['basedir'] . "/elementor/forms/" . $user_id . "/temp/" . $dropzone_hash;
